"""Two-dimensional vector geometry.
...
class Vector(tuple):
    ...
    def angle_to(self, other):
        ...
        """
        #not 'return abs(other.angle - self.angle)' because that's not in (-180, 180]
    ...
    def signed_angle_to(self, other):
        """Compute the signed angle made to another vector.
        ...
        """
        #not 'return other.angle - self.angle' because that's not in (-180, 180]
        a = other.angle - self.angle
        if a > 180:
            a -= 360
        elif a < -180:
...
class Rectangle(object):
    ...
    def as_bounding(cls, points):
        ...
        """
        xs, ys = zip(*points)
        lo = Vector((min(xs), min(ys)))
        hi = Vector((max(xs), max(ys)))
        return cls(lo, hi)
