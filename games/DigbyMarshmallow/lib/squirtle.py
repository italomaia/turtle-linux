...
loop_tess = gluNewTess()
gluTessNormal(loop_tess, 0, 0, 1)
gluTessProperty(loop_tess, GLU_TESS_WINDING_RULE, GLU_TESS_WINDING_NONZERO)
gluTessProperty(loop_tess, GLU_TESS_BOUNDARY_ONLY, GL_TRUE)

hull_tess = gluNewTess()
gluTessNormal(hull_tess, 0, 0, 1)
gluTessProperty(hull_tess, GLU_TESS_WINDING_RULE, GLU_TESS_WINDING_NONZERO)
gluTessProperty(hull_tess, GLU_TESS_BOUNDARY_ONLY, GL_TRUE)
...
class SVG(object):
    ...
    def __repr__(self):
        return "SVG(%r, anchor_x=%r, anchor_y=%r)" % (self.filename, self.anchor_x, self.anchor_y)
    ...
    def generate_disp_list(self):
        if (self.filename, self.bezier_points) in self._disp_list_cache:
            self.disp_list, self.width, self.height, self.paths = self._disp_list_cache[self.filename, self.bezier_points]
        else:
            ...
            self.render_slowly()
            glEndList()
            self._disp_list_cache[self.filename, self.bezier_points] = (self.disp_list, self.width, self.height, self.paths)

    def do_scaling(self, width, height, radius):
        if radius is not None:
            scale = 2 * radius / max(self.height, self.width)
            params = scale, scale, 1.0
            ...
            params = width / self.width, height / self.height, 1.0
        glScalef(*params)
        
    def draw(self, x, y, z=0, angle=0, scale=1.0, width=None, height=None, radius=None):
        ...
        self.do_scaling(width, height, radius)
        if scale != 1:
            try:
    ...
    def render_slowly(self):
        self.n_tris = 0
        self.n_lines = 0
        for path, stroke, tris, fill, transform, outline in self.paths:
            if tris:
    ...
    def triangulate(self, looplist):
        ...
        @set_tess_callback(tess, GLU_TESS_ERROR)
        ...
        @set_tess_callback(tess, GLU_TESS_COMBINE)
    ...
    def calc_hull(self):
        ...
        for path in self.paths:
            if path[5]:
                t_loop = []
                for loop in path[5]:
                    t_loop.append([path[4](pt) for pt in loop])
                looplist.append(t_loop)
                
        @set_tess_callback(loop_tess, GLU_TESS_VERTEX)
        def vertexLoop(vertex):
            vertex = cast(vertex, POINTER(GLdouble))
            gluTessVertex(hull_tess, vertex, vertex)
        
        @set_tess_callback(hull_tess, GLU_TESS_VERTEX)
        def vertexHull(vertex):
            vertex = cast(vertex, POINTER(GLdouble))
            self.hull_loop.append(vertex[0:2])
            
        @set_tess_callback(loop_tess, GLU_TESS_BEGIN)
        def beginLoop(which):
            gluTessBeginContour(hull_tess)
        
        @set_tess_callback(hull_tess, GLU_TESS_BEGIN)
        def beginHull(which):
            self.hull_loop = []
        
        @set_tess_callback(loop_tess, GLU_TESS_END)
        def endLoop():
            gluTessEndContour(hull_tess)
        
        @set_tess_callback(hull_tess, GLU_TESS_END)
        def endHull():
            self.hull_loops.append(self.hull_loop)
        ...
        errorLoop = set_tess_callback(loop_tess, GLU_TESS_ERROR)(errorCallback)
        errorHull = set_tess_callback(hull_tess, GLU_TESS_ERROR)(errorCallback)
        ...
        combineLoop = set_tess_callback(loop_tess, GLU_TESS_COMBINE)(combineCallback)
        combineHull = set_tess_callback(hull_tess, GLU_TESS_COMBINE)(combineCallback)
        ...
        for loop in looplist:
            ...
            data_lists.append(d_list2)
            
        gluTessBeginPolygon(hull_tess, None)
        for d_list2 in data_lists:
            gluTessBeginPolygon(loop_tess, None)
            for d_list in d_list2:
                gluTessBeginContour(loop_tess)
                for v_data in d_list:
                    gluTessVertex(loop_tess, v_data, v_data)
                gluTessEndContour(loop_tess)
            gluTessEndPolygon(loop_tess)
        gluTessEndPolygon(hull_tess)
        
    @property
    def hull(self):
        if not hasattr(self, 'hull_loops'):
            self.calc_hull()
        return self.hull_loops
           
    def warn(self, message):
        print "Warning: SVG Parser (%s) - %s" % (self.filename, message)

class SVGElement(pyglet.text.document.InlineElement):
    def __init__(self, svg, text_height, radius, width):
        self.svg = svg
        self.layout = None
        self.radius = radius
        self.height = text_height * .5
        self.width = width
        super(SVGElement, self).__init__(radius + self.height, radius - self.height, width)
    ...
    def draw(self):
        if self.layout:
            x = self.layout.x - self.layout.view_x + self.x + self.width*.5
            y = self.layout.y - self.layout.view_y + self.y + self.height
            self.svg.draw(x, y, radius=self.radius)
