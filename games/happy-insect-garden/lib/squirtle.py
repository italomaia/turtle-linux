...
class Matrix(object):
    ...
    def det(self):
        return float(self.values[0]*self.values[3] - self.values[1]*self.values[2])
    def inverse(self):
        d = self.det()
...
class SVG(object):
    ...
    def curve_to(self, x1, y1, x2, y2, x, y):
        length = math.sqrt((self.x - x1) ** 2 + (self.y - y1) ** 2)
        length += math.sqrt((x2 - x1) ** 2 + (y2 - y1) ** 2)
        length += math.sqrt((x2 - x) ** 2 + (y2 - y) ** 2)
        n_points = max(1, int(min(length*math.sqrt(abs(self.transform.det())) / 3, self.bezier_points)))
        if n_points not in self.bezier_coefficients:
            self.bezier_coefficients[n_points] = []
            for i in xrange(n_points+1):
                t = float(i)/n_points
                t0 = (1 - t) ** 3
                t1 = 3 * t * (1 - t) ** 2
                t2 = 3 * t ** 2 * (1 - t)
                t3 = t ** 3
                self.bezier_coefficients[n_points].append([t0, t1, t2, t3])
        self.last_cx = x2
        self.last_cy = y2
        for i, t in enumerate(self.bezier_coefficients[n_points]):
    ...
    def end_path(self):
        ...
        if self.path:
            ...
            if not stroke:
                stroke = self.fill
                if not isinstance(stroke, str) and stroke[3] != 255:
                    stroke = None
            self.paths.append((path if stroke else None, stroke,
